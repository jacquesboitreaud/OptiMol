...
import pandas as pd
import pickle
with open('data/drd3_scores.pickle', 'rb') as f :
    scored_smiles = pickle.load(f)

df = pd.read_csv('data/moses_train.csv', index_col= 0)
...
df.to_csv('data/moses_train.csv')
print('Saved train subset with "drd3" column. Value 0.0 indicates no data')

# test set 
df = pd.read_csv('data/moses_test.csv', index_col= 0)
smiles = df.smiles
scores = []
